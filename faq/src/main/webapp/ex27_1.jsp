...
<head>
<meta charset="UTF-8">
<title>ex27.jsp 파일 업로드 처리 파트</title>
</head>
<body>
	<%
		//JSP -> COS 파일명을 변경할 수 없음
		
		String now = new SimpleDateFormat("yyyyMMdd").format(new Date());
	
		//저장경로
		String save = "C:\\myhome2\\faq\\src\\main\\webapp\\upload";	//저장경로 
		//저장 시 POST전송 된 파일크기 용량
		int max_save =1024*1024*2;
		//파일명 인코딩
		String lang ="utf-8";
	
		MultipartRequest mr = new MultipartRequest(request,save,max_save,lang, new DefaultFileRenamePolicy());
		//MutipartRequest: 파일 저장 시 사용하는 라이브러리 클래스명
		//new DefaultFileRenamePolicy(): 새로운 객체 생성을 하기 위해서
		
		Enumeration files = mr.getFileNames(); //getFileNames() 배열정보
		//Enumeration: 파일 객체이므로 자료형으로 받음
		String filenm =(String)files.nextElement();
		//nextElement(): 업로드한 파일 모두를 지칭
		...
		if(savefile!=null){
			...
			//StandardCopyOption.ATOMIC_MOVE: JAVA언어. 기존 파일은 삭제 하고, 변경된 파일로 복사하면서 이름이 변경
		}
